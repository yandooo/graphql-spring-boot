...
class GraphQLTestTemplateTest {
    ...
    @BeforeEach
    void setUp() {
        graphQLTestTemplate = new GraphQLTestTemplate(resourceLoader, testRestTemplate, GRAPHQL_ENDPOINT,
            new ObjectMapper());
    }
    ...
    void testWithBasicAuth() {
        // WHEN
        final GraphQLTestTemplate actual = graphQLTestTemplate.withBasicAuth(TEST_USERNAME, TEST_PASSWORD);
        // THEN
        assertThat(actual).isSameAs(graphQLTestTemplate);
        assertAuthHeader(BASIC_AUTH_PREFIX, ENCODED_BASIC_AUTH);
    }
    ...
    private void assertThatContainsHeader(final String headerName, final String headerValue) {
        assertThat(graphQLTestTemplate.getHeaders()).containsEntry(headerName, Collections.singletonList(headerValue));
    }
}
